...
if [ -z "$RUBY_PACKAGE" ]; then
  ...
  sudo apt-get $APT_PROXY install ruby-full curl zlib1g-dev build-essential libssl-dev libreadline-dev libyaml-dev libsqlite3-dev sqlite3 libxml2-dev libxslt1-dev libcurl4-openssl-dev python-software-properties libffi-dev
  if [ "$?" -ne 0 ]; then
